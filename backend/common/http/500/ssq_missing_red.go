...
// SSQRedMissingData 双色球红球遗漏数据结构
type SSQRedMissingData struct {
...
// FetchSSQRedMissingData 抓取双色球红球遗漏数据
// periodCount: 期数范围，支持10/30/50
func FetchSSQRedMissingData(periodCount int) ([]SSQRedMissingData, error) {
	// 确保期数参数有效
	...
	// 构建请求URL
	url := fmt.Sprintf("https://datachart.500.com/ssq/omit/newinc/hmyl_red.php?select=%d", periodCount)
	...
	// 存储结果
	var missingData []SSQRedMissingData
...
// GetSSQRedMissingDataJSON 获取双色球红球遗漏数据并返回JSON字符串
func GetSSQRedMissingDataJSON(periodCount int) (string, error) {
	data, err := FetchSSQRedMissingData(periodCount)
	if err != nil {
		return "", err
	}
