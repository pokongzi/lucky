...
// DLTBlueMissingData 大乐透蓝球遗漏数据结构
type DLTBlueMissingData struct {
...
// FetchDLTBlueMissingData 抓取大乐透蓝球遗漏数据
// periodCount: 期数范围，支持10/30/50
func FetchDLTBlueMissingData(periodCount int) ([]DLTBlueMissingData, error) {
	// 确保期数参数有效
	...
	// 构建请求URL
	url := fmt.Sprintf("https://datachart.500.com/dlt/omit/newinc/hmyl_back.php?select=%d", periodCount)
	...
	req.Header.Set("Referer", "https://datachart.500.com/dlt/")
	...
	// 存储结果
	var missingData []DLTBlueMissingData
...
// GetDLTBlueMissingDataJSON 获取大乐透蓝球遗漏数据并返回JSON字符串
func GetDLTBlueMissingDataJSON(periodCount int) (string, error) {
	data, err := FetchDLTBlueMissingData(periodCount)
	if err != nil {
		return "", err
	}
